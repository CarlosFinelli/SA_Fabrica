            ...
            when (opcao) {
                1 -> {
                    ...
                    when (decisao) {
                        ...
                        4 -> {
                            try {
                                var carroPecas = CarroParts()
                                print("\n\nPor favor, informe o nome da peça: ")
                                carroPecas.nome = scan.nextLine()
                                carroPecas.nome = scan.nextLine()
                                print("Por favor, informe o preço dessa peça: ")
                                carroPecas.preco = scan.nextDouble()
                                print("Por favor, informe o tipo desta peça: ")
                                carroPecas.tipo = scan.nextLine()
                                if (listPecas.size < 1) {
                                    carroPecas.codPeca = 1
                                } else {
                                    var ultimaPeca = listPecas.last()
                                    carroPecas.codPeca = ultimaPeca.codPeca + 1
                                }
                                listPecas.add(carroPecas)
                            } catch (e: IOException) {
                                println("\n\nAlgum erro ocorreu.")
                                e.stackTrace
                                continue
                        ...
                        5 -> {
                            try {
                                ...
                                print("\nPor favor, insira o código da peça que deseja editar:")
                                var codigo = scan.nextInt()
                                var confereCodigo = listPecas.find { item -> item.codPeca == codigo }
                                if (confereCodigo != null) {
                                    println("\n\n\n1 - Nome")
                                    println("2 - Preço")
                                    println("3 - Tipo")
                                    println("4 - Voltar a página inicial")
                                    print("\nPor favor, selecione uma opção: ")
                                    var decisao = scan.nextInt()
                                    when (decisao) {
                                        1 -> {
                                            print("Por favor, insira o novo nome da peça: ")
                                            confereCodigo.nome = scan.nextLine()
                                            confereCodigo.nome = scan.nextLine()
                                            continue
                                        }

                                        2 -> {
                                            print("Por favor, insira o novo preço da peça: ")
                                            confereCodigo.preco = scan.nextDouble()
                                            continue
                                        }

                                        3 -> {
                                            print("Por favor, informe o novo tipo desta peça: ")
                                            confereCodigo.tipo = scan.nextLine()
                                            confereCodigo.tipo = scan.nextLine()
                                            continue
                                        }
                        ...
                        6 -> {
                            try {
                                ...
                                print("\nPor favor, insira o código da peça que deseja remover:")
                                var codigo = scan.nextInt()
                                var confereCodigo = listPecas.find { item -> item.codPeca == codigo }
                                if (confereCodigo != null) {
                                    listPecas.remove(confereCodigo)
                                } else {
                                    println("Código inválido.")
                                    continue
                                }
                        ...
                        7 -> {
                            try {
                                ...
                                print("Informe o CPF do cliente que deseja agendar a manutenção: ")
                                var cpfPesquisa = scan.nextLine()
                                cpfPesquisa = scan.nextLine()
                                var cli = listClientes.find {item -> item.cpf == cpfPesquisa}
                                if (cli != null) {
                                    manutencao.cpfRequisitante = cli.cpf
                                    print("Por favor, informe a data da manutenção \"Formato: DD/MM/AAAA, favor digitar as /\": ")
                                    manutencao.dataManutencao = scan.nextLine()
                                    var escolha = 0
                                    while (escolha != 2) {
                                        println("\n")
                                        listPecas.forEach {
                                            println("| Codígo: ${it.codPeca} | Nome: ${it.nome} |")
                                        }
                                        print("Selecione o código da peça que deseja incluir: ")
                                        var peca = scan.nextInt()
                                        var encontraPeca = listPecas.find{item -> item.codPeca == peca}
                                        if (encontraPeca != null) {
                                            manutencao.listPecas.add(encontraPeca)
                                            print("Deseja adicionar mais alguma peça?\"1 = Sim, 2 = Não\": ")
                                            escolha = scan.nextInt()
                                        } else {
                                            println("Código inválido")
                                        }
                                    }
                                    listManutencoes.add(manutencao)
                                    continue
                                }  else {
